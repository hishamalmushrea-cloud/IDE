...
package com.aidenext.javac.services.partial

import com.aidenext.javac.services.compiler.JavacFlowListener
import com.aidenext.javac.services.util.ReparserUtils
import com.aidenext.javac.services.visitors.FindAnonymousVisitor
import com.aidenext.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = com.aidenext.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
      com.aidenext.javac.services.NBParserFactory.instance(context)
        as com.aidenext.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = com.aidenext.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
      endPositions
        is com.aidenext.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
      com.aidenext.javac.services.NBParserFactory.NBJavacParser(
        factory,
        lexer,
        true,
        false,
